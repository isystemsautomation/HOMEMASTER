...
// ================== GPIO MAP (WLD-521-R1) ==================
// Five digital inputs on GPIO3, GPIO2, GPIO8, GPIO9, GPIO15 (DI1..DI5)
static const uint8_t DI_PINS[5]    = {3, 2, 8, 9, 15};   // DI1..DI5
// Two relays RELAY1 (GPIO0), RELAY2 (GPIO1), active-HIGH
...
// ================== Config & runtime ==================
struct InCfg  { bool enabled; bool inverted; uint8_t action; /*0=None,1=Toggle,2=Pulse*/ uint8_t target; /*4=None,0=All,1..2=R1..R2*/ };
struct RlyCfg { bool enabled; bool inverted; };

InCfg   diCfg[NUM_DI];
RlyCfg  rlyCfg[NUM_RLY];

bool diState[NUM_DI]        = {false,false,false,false,false};
bool diPrev[NUM_DI]         = {false,false,false,false,false};

// Desired relay state (PLC/command, DI actions set this)
...
static const uint16_t CFG_VERSION = 0x0001;       // WLD-521-R1 layout v1
...
// ================== Modbus state (FC=02) ==================
enum : uint16_t {
  ISTS_DI_BASE   = 1,   // 1..5 : IN1..IN5 (after enable+invert)
  ISTS_RLY_BASE  = 60   // 60..61 : RELAY1..2 logical state
};

// ================== Modbus command coils (FC=05/15; pulse) ==================
enum : uint16_t {
  CMD_RLY_ON_BASE   = 200,  // 200..201 : pulse turn Relay1..2 ON
  CMD_RLY_OFF_BASE  = 210,  // 210..211 : pulse turn Relay1..2 OFF
  CMD_DI_EN_BASE    = 300,  // 300..304 : pulse ENABLE  IN1..IN5
  CMD_DI_DIS_BASE   = 320   // 320..324 : pulse DISABLE IN1..IN5
};
...
void setup() {
  ...
  WebSerial.send("message", "Boot OK (DI actions: None/Toggle/Pulse; targets: None/All/R1/R2)");
  sendAllEchoesOnce();
}
...
// Supported types: inputEnable, inputInvert, inputAction, inputTarget, relays
void handleValues(JSONVar values) {
...
void loop() {
  ...
  for (int i = 0; i < NUM_RLY; i++) {
    ...
    // End any pulse whose window expired
  ...
  if (millis() - lastSend >= sendInterval) {
    ...
    for (int i = 0; i < NUM_DI; i++) {
      ...
      targetList[i] = diCfg[i].target;     // 4=None,0=All,1..2=R1..R2
    }
